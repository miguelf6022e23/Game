...
def display_box(screen, message,gameDisplay):
  ...
  pygame.display.flip()
#-------------------display text---------------------------------
def dispT(x,yo,message,gameDisplay):
    fontobject = pygame.font.Font(None,18)
    screen = gameDisplay
    if len(message) != 0:
...
#--------display tree-------------------------------------
def dispTree(root,x,x0,y0,dispW,dispH,xl,yl,gameDisplay):
    global y
    ...
    if type(root) is textNode:
        dispT(x+x0+dispW/50,y+y0,root.t,gameDisplay)
    elif (type(root) is flagCheck) or (type(root) is flagSet):
        dispT(x+x0+dispW/50,y+y0,root.i,gameDisplay)
    elif type(root) is decNode:
        for i in range(len(root.a)+1):
            ...
            if i==0:
                dispT(x+x0+dispW/50,y+y0,root.q,gameDisplay)
            else:
                dispT(x+x0+dispW/50,y+y0+dispH*(i)/(4*(len(root.a)+1)),root.a[i-1],gameDisplay)
    if type(root) is flagSet:
        dispT(x+x0+dispW/50,y+y0+dispH/8,str(root.b),gameDisplay)
    for i in range(len(root.n)):
        if i > 0:
            y = y+dispH/3
        dispTree(root.n[i],x+dispW/3,x0,y0,dispW,dispH,x+x0+dispW/4,temp+y0+dispH/8,gameDisplay)
#--------left click------------------------------------
def lc(root,pos,x,x0,y0,dispW,dispH,gameDisplay):
    global y
    #change node type
    ...
    for i  in range(len(root.n)):
        if i>0:
            y=y+dispH/3
        root.n[i] = lc(root.n[i],pos,x+dispW/3,x0,y0,dispW,dispH,gameDisplay)
    return root
#vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
#Main function
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
def dialogueV2(sf,rootN,screenInfo,gameDisplay,clock,dispW,dispH):
    global y
    y = 0
    #--------interaction loop-----------------------------------------
    done = False
    ...
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                root = lc(root,pos,0,x0,y0,dispW,dispH,gameDisplay)
                y = 0
        ...
        dispTree(root,0,x0,y0,dispW,dispH,x0,y0,gameDisplay)
        y = 0
